...
fn main() -> Result<(), Error> {
	...
	let est_buf_len = {
		...
		(md_est + toml_est) as usize
	};

	let mut file_buf = String::with_capacity(est_buf_len);

	let metadata = parse_toml(&mut file_buf, &toml_path)?;

	println!("With metadata:\n{metadata:?}");

	let timestamp = chrono::Local::now();

	let buf_w_yaml = compile_yaml_metadata(file_buf, &metadata, &parsed_name.1, &timestamp);

	let input_md_content =
		std::fs::read_to_string(&input_md_path).map_err(|e| Error::ReadInputMd(e))?;

	let first_line = input_md_content.lines().next();
	let first_line_len = match first_line {
		Some(content) => {
	...
	let mut buf_w_std_sections = compile_std_sections(
		buf_w_yaml,
		&metadata,
		&parsed_name.1,
		gdoc.revision,
	...
	buf_w_std_sections.write_str("# Objectives {-}").unwrap();
	buf_w_std_sections
		.write_str(&input_md_content[first_line_len..])
		.unwrap();
